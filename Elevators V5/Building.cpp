...
bool Building::boardRider(int e) 
{
    ...
    if (elevators[e].isOpen() && !elevators[e].isFull())
    {
        ...
        /*
        if (elevators[e].goingUp()) {
            cout << "UP";
        }
        else if (elevators[e].goingDown()) {
        ...
        if (floor.hasUpRider()) {
            ...
            cout << "Who knows";
            cout << floor.getNumberOfDownRiders() << floor.getNumberOfUpRiders();
        }
        cout << endl;
        */
...
bool Building::waitingForMoreRiders(int e) {
    if (elevators[e].isOpen() || elevators[e].isIdle() == false || elevators[e].timedOut() == false) {
        return false;
    }
...
bool Building::doneWaitingMove(int e) {
    if (elevators[e].isOpen() == false || elevators[e].isIdle() || elevators[e].timedOut() == false) {
        return false;
    }
...
bool Building::setIdleElevatorInMotion(int e) 
{
    if (elevators[e].goingUp() || elevators[e].goingDown()) 
    {
        ...
        for (int i = 0; i < FLOORS; i++) {
            if (elevators[e].getLocation() == floors[i].elevation)
            {
                continue;
            }
            if (floors[i].panel.isLit("UP")) {
                elevators[e].openDoorTo(i);
                elevators[e].setDirectionUp();
                return true;
            }
            else  if (floors[i].panel.isLit("DOWN"))
            {
                elevators[e].openDoorTo(i);
...
bool Building::sendIdleElevatorToCallingFloor(int e)
{
    if (elevators[e].goingUp() || elevators[e].goingDown()) 
    {
        ...
        for (int i = 0; i < FLOORS; i++) {
            if (floors[i].panel.isLit("UP") == false && floors[i].panel.isLit("DOWN") == false)
            {
                continue;
            }
            if (floors[i].elevation > elevators[e].getLocation()) 
            {
                elevators[e].setDirectionUp();
                elevators[e].move();
                return true;
